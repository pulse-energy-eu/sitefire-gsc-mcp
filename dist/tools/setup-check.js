...
export async function setupCheck(client, authState, siteUrl) {
    const next_actions = [];
    ...
    if (propertyUrls.includes(targetSite)) {
        ...
        next_actions.push(`The property ${targetSite} is not accessible under this Google account. Run list_my_properties to see what you do have.`);
        return {
            auth_valid: true,
            properties_count: propertyUrls.length,
            properties: propertyUrls,
    ...
    try {
        ...
        if (sitemaps.length === 0) {
            next_actions.push("Submit a sitemap at search.google.com/search-console under the Sitemaps section.");
        }
    }
    catch {
    ...
    try {
        ...
        if (analytics.rows.length === 0) {
            ...
            next_actions.push("Your property is added but Google hasn't collected data yet. Typical wait: 2-7 days.");
        }
        else {
            ...
            if (totalClicks === 0) {
                next_actions.push("Google has data for your property but no clicks in the last 28 days. This is normal for new or low-traffic sites.");
            }
        }
    }
    catch {
