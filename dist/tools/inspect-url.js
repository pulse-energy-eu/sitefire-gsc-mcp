 ...
 */
function computeCrawlAgeDays(lastCrawlTime) {
    if (!lastCrawlTime)
        return null;
...
function deriveInterpretation(idx) {
    const robotsBlocked = idx.robotsTxtState === "DISALLOWED" ||
    ...
    if (idx.googleCanonical &&
        ...
        return {
            ...
            recommended_action: "If this URL should be the canonical, ensure it has unique content, proper internal linking, and a self-referencing canonical tag. Consolidate duplicate content.",
        };
    }
    // Indexed
    if (idx.verdict === "PASS") {
    ...
    if (idx.coverageState?.includes("Crawled") &&
        idx.coverageState?.includes("not indexed")) {
        return {
            state: "crawled_not_indexed",
            summary: "Google crawled this URL but decided not to index it. This often means the content is thin, duplicative, or low-value relative to other pages.",
            recommended_action: "Improve the content quality, add unique value, ensure proper internal linking, and request re-indexing in Search Console.",
        };
    }
    // Fallback
    return {
        ...
        recommended_action: "Check the URL in Search Console's URL Inspection tool for more details.",
    };
}
export async function inspectUrlTool(client, siteUrl, url) {
    ...
    const robotsAllowed = idx.robotsTxtState !== "DISALLOWED" &&
        idx.robotsTxtState !== "BLOCKED_BY_ROBOTS_TXT";
    ...
    const canonicalMismatch = googleCanonical !== null &&
        userCanonical !== null &&
    ...
    return {
        ...
        crawled_as: idx.crawledAs ?? "MOBILE",
