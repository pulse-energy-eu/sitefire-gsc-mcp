...
function selectStrikingDistance(rows) {
    return rows
...
function selectLowHangingFruit(rows) {
    if (rows.length === 0)
        return [];
    ...
    return rows
        .filter((r) => r.impressions > medianImpressions &&
        r.ctr < avgCtr &&
        r.position <= 10)
        .sort((a, b) => b.impressions - a.impressions)
        .slice(0, 15)
...
function getMedianImpressions(rows) {
...
export async function findOpportunities(client, siteUrl, days = 28) {
    ...
    const allEmpty = strikingQueries.length === 0 &&
...
//# sourceMappingURL=find-opportunities.js.map