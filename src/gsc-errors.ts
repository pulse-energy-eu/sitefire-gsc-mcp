...
export class ReauthRequired extends GscApiError {
  constructor(detail?: string) {
    super(
      "REAUTH_REQUIRED",
      "Your Google authorization was revoked or expired. Run setup_check to re-authenticate.",
      401,
      detail,
    );
 ...
 */
export function translateError(err: unknown, context?: { siteUrl?: string; url?: string }): GscApiError {
  // Already translated
  if (err instanceof GscApiError) return err;
  ...
  if (httpStatus === 403) {
    if (
      ...
      return new GscApiError(
        "PERMISSION_DENIED",
        `You don't have access to${siteHint}. Run list_my_properties to see what you do have, or sign in with a different Google account.`,
        403,
        message,
      );
    }
    if (message.includes("insufficientPermissions") || reason === "insufficientPermissions") {
      return new GscApiError(
        "INSUFFICIENT_SCOPE",
        "This action requires more access than you granted. v0 is read-only; upgrade coming soon.",
        403,
        message,
      );
    }
    ...
    return new GscApiError(
      "PERMISSION_DENIED",
      `Access denied. Run list_my_properties to see your accessible properties.`,
      403,
      message,
    );
  }
  ...
  if (httpStatus === 400) {
    ...
    return new GscApiError(
      "INVALID_ARGUMENT",
      `The property${siteHint} isn't in the right format. Run list_my_properties to see the exact strings Google uses.`,
      400,
      message,
    );
  }
  ...
  if (err instanceof Error) {
    if (
      ...
      return new GscApiError(
        "NETWORK_ERROR",
        "Couldn't reach Google. Check your internet connection.",
        undefined,
        message,
      );
    }
