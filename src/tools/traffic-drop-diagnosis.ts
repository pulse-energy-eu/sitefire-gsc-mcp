...
import { gscComparisonRanges, type DateRange } from "./shared/gsc-dates.js";
...
export interface DropDiagnosis {
  site_url: string;
  compare: { current_period: DateRange; previous_period: DateRange };
  overall_change: {
    clicks: number;
    impressions: number;
    position: number;
  ...
  top_query_losses: Array<QueryMetrics & { delta: Delta }>;
  top_page_losses: Array<PageMetrics & { delta: Delta }>;
...
const GSC_NOTE =
  "Google Search Console data has a 2-3 day freshness lag and is sampled. " +
...
function toQueryMap(rows: SearchAnalyticsRow[]): Map<string, SearchAnalyticsRow> {
...
function computeQueryLosses(
  ...
  const losses: Array<QueryMetrics & { delta: Delta }> = [];
  ...
  for (const key of allKeys) {
    ...
    if (clickDelta >= 0) continue; // not a loss
...
function computePageLosses(
  currentRows: SearchAnalyticsRow[],
  previousRows: SearchAnalyticsRow[],
): Array<PageMetrics & { delta: Delta }> {
  const prevMap = toQueryMap(previousRows); // same key structure works for pages
  const currentMap = toQueryMap(currentRows);

  const allKeys = new Set([...prevMap.keys(), ...currentMap.keys()]);

  const losses: Array<PageMetrics & { delta: Delta }> = [];
...
function computeOverallChange(
  ...
  const sumClicks = (rows: SearchAnalyticsRow[]) =>
    rows.reduce((s, r) => s + r.clicks, 0);
  const sumImpressions = (rows: SearchAnalyticsRow[]) =>
    rows.reduce((s, r) => s + r.impressions, 0);
  const avgPosition = (rows: SearchAnalyticsRow[]) => {
...
function attributeRootCause(
  overallChange: { clicks: number; impressions: number; position: number },
  queryLosses: Array<QueryMetrics & { delta: Delta }>,
  currentQueryRows: SearchAnalyticsRow[],
  previousQueryRows: SearchAnalyticsRow[],
): { primary: RootCause; confidence: "high" | "medium" | "low"; explanation: string } {
  ...
  if (currentQueryRows.length === 0 && previousQueryRows.length === 0) {
    return {
      ...
      confidence: "high",
      explanation:
        "No search data available for either period. This property may " +
  ...
  if (overallChange.clicks >= 0) {
    return {
      ...
      confidence: "high",
      explanation:
        "No traffic drop detected in the comparison period. " +
  ...
  if (totalClickLoss > 0 && queryLosses.length > 0) {
    ...
    if (top3AbsLoss / totalClickLoss > 0.6) {
      ...
      return {
        ...
        confidence: "high",
        explanation:
          `A small number of queries (${topQueries}) account for most of ` +
  ...
  if (overallChange.position > 2) {
    return {
      ...
      confidence: "medium",
      explanation:
        `Average position worsened by ${overallChange.position.toFixed(1)} ` +
  ...
  if (
    ...
    return {
      ...
      confidence: "medium",
      explanation:
        "Impressions dropped significantly while rankings stayed similar. " +
  ...
  if (
    ...
    return {
      ...
      confidence: "low",
      explanation:
        "Clicks and impressions dropped at roughly the same rate, which " +
  ...
  return {
    ...
    confidence: "low",
    explanation:
      "Traffic declined but no single root cause stands out clearly. " +
...
export async function trafficDropDiagnosis(
  client: GscClient,
  siteUrl: string,
  comparePeriod: ComparePeriod = "wow",
): Promise<DropDiagnosis> {
  ...
  const extractRows = (r: PromiseSettledResult<{ rows: SearchAnalyticsRow[] }>) =>
  ...
  const rootCause = attributeRootCause(
    overallChange,
    queryLosses,
    currentQueryRows,
    previousQueryRows,
