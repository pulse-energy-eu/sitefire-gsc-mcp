...
const EMPTY_STATE_GUIDANCE =
  "This property is too fresh to show search data. Google typically populates GSC within 2-7 days of first crawl. Run setup_check for a status summary, or come back in a few days.";
...
export async function weeklyReport(
  ...
  if (partialFailures.length === 5) {
    ...
    if (firstApiError) throw firstApiError;
  }
...
function buildRollup(
  currentAgg: SearchAnalyticsResult | null,
  byQuery: SearchAnalyticsResult | null,
  byPage: SearchAnalyticsResult | null,
): WeeklyReport["rollup"] {
  if (!currentAgg || currentAgg.rows.length === 0) return null;
...
function buildWowDelta(
  currentAgg: SearchAnalyticsResult | null,
  previousAgg: SearchAnalyticsResult | null,
): WeeklyReport["wow_delta"] {
  if (!currentAgg || currentAgg.rows.length === 0) return null;
  if (!previousAgg || previousAgg.rows.length === 0) return null;

  const curr = currentAgg.rows[0];
  const prev = previousAgg.rows[0];

  // Avoid division by zero
  if (prev.clicks === 0 && prev.impressions === 0) return null;
...
function buildTopQueries(byQuery: SearchAnalyticsResult | null): QueryMetrics[] {
  if (!byQuery) return [];
...
function buildTopPages(byPage: SearchAnalyticsResult | null): PageMetrics[] {
  if (!byPage) return [];
...
function buildSitemapHealth(sitemaps: SitemapEntry[]): WeeklyReport["sitemap_health"] {
  let errors = 0;
  let warnings = 0;
